...
var debtorsWithMatchingEmailDomains = debtors.Where(d => d.Email.EndsWith("rhyta.com") || d.Email.EndsWith("dayrep.com")).ToList();
debtorsWithMatchingEmailDomains.ForEach(d => Console.WriteLine(d));

// Task3: Yashi 26-dan 36-ya qeder olan borclulari cixartmag
var debtorsBetween26And36YearsOld = debtors.Where(d =>
{
    int age = DateTime.Now.Year - d.BirthDay.Year;
    return age >= 26 && age <= 36;
}).ToList();
debtorsBetween26And36YearsOld.ForEach(d => Console.WriteLine(d));

// Task4: Borcu 5000-den cox olmayan borclularic cixartmag
var debtorsWithDebtLessThan5000 = debtors.Where(d => d.Debt < 5000).ToList();
debtorsWithDebtLessThan5000.ForEach(d => Console.WriteLine(d));

// Task5: Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2-den cox 7 reqemi olan borclulari cixartmaq
...
// Task7: Qishda anadan olan borclulari cixardmaq
var winterBornDebtors = debtors.Where(d => d.BirthDay.Month == 12 || d.BirthDay.Month == 1 || d.BirthDay.Month == 2).ToList();
winterBornDebtors.ForEach(d => Console.WriteLine(d));

// Task8: Borcu, umumi borclarin orta borcunnan cox olan borclulari cixarmaq ve evvel familyaya gore sonra ise meblegin azalmagina gore sortirovka etmek
...
// Task9: Telefon nomresinde 8 olmayan borclularin yalniz familyasin, yashin ve umumi borcun meblegin cixarmaq
...
// Task11: Adinda ve familyasinda hec olmasa 3 eyni herf olan borclularin siyahisin cixarmaq ve onlari elifba sirasina gore sortirovka elemek
var debtorsWithThreeSameLettersInName = debtors.Where(d => d.FullName.Distinct().Count() <= d.FullName.Length - 3).OrderBy(d => d.FullName).ToList();
debtorsWithThreeSameLettersInName.ForEach(d => Console.WriteLine(d));

// Task13: borclulardan en coxu hansi ilde dogulubsa hemin ili cixartmaq
var mostCommonBirthYear = debtors.GroupBy(d => d.BirthDay.Year).OrderByDescending(g => g.Count()).First().Key;
Console.WriteLine($"Most common birth year among debtors: {mostCommonBirthYear}");

// Task14: Borcu en boyuk olan 5 borclunun siyahisini cixartmaq
var top5Debtors = debtors.OrderByDescending(d => d.Debt).Take(5).ToList();
top5Debtors.ForEach(d => Console.WriteLine(d));

// Task15: Butun borcu olanlarin borcunu cemleyib umumi borcu cixartmaq
var totalDebt = debtors.Sum(d => d.Debt);
Console.WriteLine($"Total debt of all debtors: {totalDebt}");

// Task16: 2ci dunya muharibesin gormush borclularin siyahisi cixartmaq
...
// Task19: Tesevvur edek ki,butun borclari olanlar bugunden etibaren her ay 500 azn pul odeyecekler. Oz ad gunune kimi borcun oduyub qurtara bilenlerin siyahisin cixartmaq
var debtorsWithAllDebtsPaid = debtors.Where(d => d.Debt <= (DateTime.Now.Year - d.BirthDay.Year) * 500).ToList();
debtorsWithAllDebtsPaid.ForEach(d => Console.WriteLine(d));

// Task20: Adindaki ve familyasindaki herflerden "smile" sozunu yaza bileceyimiz borclularin siyahisini cixartmaq
string keyword = "smile";
var debtorsWithTargetkeyword = debtors.Where(debtor =>
{
    string fullName = debtor.FullName.ToLower();
    char[] targetChars = keyword.ToCharArray();

    foreach (char c in targetChars)
    {
        if (fullName.Count(ch => ch == c) < keyword.Count(ch => ch == c))
        {
            return false;
        }
    }
...
foreach (var debtor in debtorsWithTargetkeyword)
{
